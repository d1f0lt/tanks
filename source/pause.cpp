...
Pause::Pause() : background(sf::Vector2<float>(WINDOW_WIDTH, WINDOW_HEIGHT)) {
    ...
    // Header
    items.emplace_back(std::make_unique<PauseItem>(path + "pause.png", currentCoordinates));
    items[0]->inscriptionSprite.setPosition({static_cast<float>(static_cast<int>(currentCoordinates.x) + static_cast<int>((pauseWidth - items[0]->inscriptionImage.getSize().x) / 2)), currentCoordinates.y}); // Centralize

    // Buttons
    currentCoordinates.y += paddingBetweenButtons * 12;
    for (int i = 2; i <= elementsCount; ++i) {
        items.emplace_back(std::make_unique<PauseButton>(path + "item_" + std::to_string(i) + ".png", currentCoordinates));
        currentCoordinates.y += PauseButton::rectangleHeight + paddingBetweenButtons;
    }
}
